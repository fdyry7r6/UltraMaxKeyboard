...
import helium314.keyboard.toptopy.TopTopyStudioActivity;
import helium314.keyboard.toptopy.TopTopyTextStyles;
...
public class LatinIME extends InputMethodService implements
    ...
    private final BroadcastReceiver mTopTopyStudioReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            if (!TopTopyStudioActivity.ACTION.equals(intent.getAction())) return;
            final android.view.inputmethod.InputConnection ic = getCurrentInputConnection();
            if (ic == null) return;
            if (intent.hasExtra(TopTopyStudioActivity.EXTRA_TEXT)) {
                ic.commitText(intent.getStringExtra(TopTopyStudioActivity.EXTRA_TEXT), 1);
                return;
            }
            final String styleName = intent.getStringExtra(TopTopyStudioActivity.EXTRA_STYLE);
            if (styleName == null) return;
            TopTopyTextStyles.Style style;
            try { style = TopTopyTextStyles.Style.valueOf(styleName); } catch (Exception e) { return; }
            CharSequence selected = ic.getSelectedText(0);
            String source = selected != null ? selected.toString() : null;
            if (source == null || source.isEmpty()) {
                ...
                source=b.substring(i);
                if (source.isEmpty()) return;
                ic.deleteSurroundingText(source.length(), 0);
            }
            ic.commitText(TopTopyTextStyles.transform(source, style), 1);
        }
    };
    ...
    public void onCreate() {
        ...
        IntentFilter topTopyFilter = new IntentFilter(TopTopyStudioActivity.ACTION);
        ContextCompat.registerReceiver(this, mTopTopyStudioReceiver, topTopyFilter, ContextCompat.RECEIVER_NOT_EXPORTED);
    ...
    public void onDestroy() {
        ...
        unregisterReceiver(mTopTopyStudioReceiver);
    ...
    public void launchTopTopyStudio() {
        Intent intent = new Intent(this, TopTopyStudioActivity.class);
